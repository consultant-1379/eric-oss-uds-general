...
public class RequestHeadersValidator {
    ...
    public static void validateContentType(HttpServletRequest request, MediaType expectedContentType, Map<String, String> headersMap)
        ...
        if (contentType == null || !contentType.contains(MediaType.APPLICATION_JSON)) {
            throw new RestRequestValidationException("Content-Type of requset is different then " + expectedContentType);
        } else {
            headersMap.put(Constants.CONTENT_TYPE_HEADER, contentType);
        }
    }

    public static void validateIdentificationHeaders(HttpServletRequest request, List<String> identificationList, Map<String, String> headersMap)
        throws RestRequestValidationException {
