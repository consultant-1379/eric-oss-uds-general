...
public class FunctionalInterfaces {
     ...
     * <br>
     * In case Exception Occurred replaces it with FunctionalAttException. <br> This is useful for two cases:<br> 1.using methods that throws
     * exceptions in streams.<br> 2.replacing declared exception with undeclared exception (Runtime).<br> See below Use Case:<br> Instead of:
     * intList.stream().map(e -> fooThrowsAndReturnsBoolean(e)); - does not compile !<br> Use This : intList.stream().map(e -> swallowException( () ->
     ...
     * <br>
     * In case Exception Occurred replaces it with FunctionalAttException. <br> This is useful for two cases:<br> 1.using methods that throws
     * exceptions in streams.<br> 2.replacing declared exception with undeclared exception (Runtime).<br> See below Use Case:<br>
     *
     * @param methodToRun
     */
    ...
    /**
     * Runs the given method.<br> Verify the method result against the resultVerifier.<br> If verification passed returns the result.<br> If
     * Verification failed keeps retrying until it passes or until 10 seconds pass.<br> If Exception Occurred keeps retrying until it passes or until
     * 10 seconds pass,<br> If last retry result caused an exception - it is thrown.
     *
    ...
    public static <R> R retryMethodOnResult(Supplier<R> methodToRun, Function<R, Boolean> resultVerifier) {
        return retryMethodOnResult(methodToRun, resultVerifier, DEFAULT_MAX_WAIT_TIME_MS, DEFAULT_REDO_INTERVAL_TIME_MS);
    }

    /**
     * Runs the given method.<br> Verify the method result against the resultVerifier.<br> If verification passed returns the result.<br> If
     * Verification failed keeps retrying until it passes or until maxWait pass.<br> If Exception Occurred keeps retrying until it passes or until
     * maxWait pass,<br> If last retry result caused an exception - it is thrown.
     *
     * @param methodToRun     given Method
     * @param resultVerifier  verifier for the method result
     * @param maxWaitMS
     ...
     * @return
     */
    public static <R> R retryMethodOnResult(Supplier<R> methodToRun, Function<R, Boolean> resultVerifier, long maxWaitMS, long retryIntervalMS) {
    ...
    /**
     * Runs the given method.<br> Verify the method result against the resultVerifier.<br> If verification passed returns the result.<br> If
     * Verification failed keeps retrying until maxRetries reached.<br> If Exception Occurred keeps retrying until it passes or until maxRetries
     * reached,<br> If last retry result caused an exception - it is thrown.
     *
     * @param methodToRun    given Method
     * @param resultVerifier verifier for the method result
     * @param maxRetries
     * @return
     */
    public static <R> R retryMethodOnResult(Supplier<R> methodToRun, Function<R, Boolean> resultVerifier, long maxRetries) {
        boolean stopSearch = false;
        R ret = null;
        int retriesCount = 0;
        FunctionalAttException functionalExceotion = null;
    ...
    public static <R> R retryMethodOnException(SupplierThrows<R, Exception> methodToRun, Function<Exception, Boolean> exceptionVerifier,
                                               long maxRetries) throws Exception {
        boolean stopSearch = false;
        R ret = null;
        int retriesCount = 0;
    ...
    /**
     * Runs the given method.<br> In case exception occurred runs the method again either until succeed or until 10 seconds pass.
     *
     * @param methodToRun given method
     * @return
     */
    ...
    /**
     * Same as Thread.sleep but throws a FunctionalAttException (RuntimeException) instead of InterruptedException.<br>
     *
     * @param millis
     */
    ...
    /**
     * Converts Either containing right value to another either with different type of left value and the same type of right value.
     *
    ...
    /**
     * Converts Either containing left value to another either with different type of right value and the same type of left value.
     *
     * @param eitherToConvert
     * @return
     */
     ...
     * @return
     */
    public static <T extends Enum<T>> T getEnumValueByFieldValue(String fieldValue, T[] values, Function<T, String> enumValueGetter, T defaultValue) {
    ...
    /**
     * This method runs the given method.<br> In case given method finished running within timeoutInMs limit it returns Either which left value is the
     * result of the method that ran.<br> In case given method did not finish running within timeoutInMs limit it returns Either which right value is
     * false. <br>
     *
     * @param supplier
    ...
    /**
     * Wraps the execution of the Runnable with try catch.<br> In case exception occurred returns Optional containing the resultOnException.<br>
     ...
     * @return
     */
    public static <T, E extends Exception> Optional<T> wrapWithTryCatch(RunnableThrows<E> runnable, T resultOnException) {
    ...
    /**
     * Runs the given method.<br> In case the method passes without any Assertion Errors finishes.<br> In case there is an assertion error keeps
     * running the method every  retryIntervalMS.<br> until there is no Errors or maxWaitTimeMs has passed. <br> If there are still Assertion Errors
     * in the last Run they are returned to the user.<br>
     *
     * @param methodToRun
     * @param maxWaitTimeMs
     * @param retryIntervalMS
    ...
    /**
     * Wraps the execution of the Runnable with try catch.<br> In case exception occurred logs the Exception.<br> resultOnException.<br> Otherwise
